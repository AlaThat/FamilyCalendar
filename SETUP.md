...
- **Weekly dinner plan** — a "Dinner" text box for each of the 7 days sits right above the calendar grid in **Week view only** (not Month or Day), so paging to next/last week with the same arrows you already use for the calendar doubles as browsing your meal-plan history or planning ahead. Type a plan for any day and tap elsewhere (or the next field) to save it; clearing a box back to blank removes that day's plan. Breakfast/lunch aren't tracked — just dinner.
- **Ingredients + a reusable meal library** — tap the small button next to any day's Dinner box (it shows a "+" or the ingredient count) to open a fuller editor: add/remove ingredients, and tap "Add to list" on any of them to push it straight to the Shopping tab (handy for "we're out of pickles this week"). If a meal is one you make often, check "Save this as a reusable meal" before saving — next time, open any day's editor and pick it from the "Use a saved meal" dropdown to pull in its name and full ingredient list again, then tweak as needed for that week. Saving isn't automatic on every dinner you type — only checking that box adds it to your saved list, so one-off dinners (leftovers, takeout) don't clutter it.
