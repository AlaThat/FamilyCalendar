...
- **Egg tracker** — Day view has a small "🥚 Eggs collected" stepper for whichever date you're viewing, so you can log today's count or go back and fix a day you forgot, plus a running total for the year right next to it. Any day with eggs logged also gets a small number badge on Month/Week view (in its own corner, away from the moon phase icon, so the two never crowd each other on a day both happen to land on).
