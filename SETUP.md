...
- **Reminders can be one-time, daily, or recurring** (weekly on any combination of days, monthly, monthly-on-the-1st-Tuesday-style, or yearly — the same options as calendar events), all from one "+ Add" flow on the Reminders tab. Unlike routines/chores, a reminder doesn't reset every day — once due, it stays on the list until you check it off, however many days that takes, then clears the next day. Checking one off also drops a small "✓ done" note on the calendar for whichever day you actually did it, even if that's later than it was originally due.
